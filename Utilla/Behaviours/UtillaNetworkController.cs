...
internal class UtillaNetworkController : MonoBehaviourPunCallbacks
{
    ...
    public override void OnEnable()
    {
        ...
        if (NetworkSystem.Instance is NetworkSystem netSys && netSys is NetworkSystemPUN &&
            PhotonNetwork.NetworkingClient is LoadBalancingClient client)
        {
    ...
    {
        if (ApplicationQuittingState.IsQuitting                          || !NetworkSystem.Instance.InRoom ||
            NetworkSystem.Instance.GameModeString is not string gameMode || gameMode == null) return;
